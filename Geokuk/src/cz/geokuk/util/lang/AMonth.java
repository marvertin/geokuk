        ...
        implements IElementInt, Comparable<AMonth>, Ordinable<AMonth>
{
  private static final long serialVersionUID = 6216726658054355928L;
    ...
    {
          return MessageFormat.format("{0,number,0000}-{1,number,00}", new Object[]
                   { new Integer(iRokMes / MINY),
                     new Integer(iRokMes % MINY + 1)});
    }
     ...
     */
    public ADate firstDate(int aDay) {  return dateOf(1);  }
    ...
    public int diff(AMonth aMonth)
    {
      ...
      public boolean isLess            (AMonth b) { return compareTo(b) < 0; }
      public boolean isLessOrEqual     (AMonth b) { return compareTo(b) <= 0; }
      public boolean isGreater         (AMonth b) { return compareTo(b) > 0; }
      public boolean isGreaterOrEqual  (AMonth b) { return compareTo(b) >= 0; }
      public boolean isEqual           (AMonth b) { return compareTo(b) == 0; }
      public boolean isNotEqual        (AMonth b) { return compareTo(b) != 0; }
    ...
    public int hashCode()
    {
            return new Integer(iRokMes).hashCode();
    }
   ...
   public AMonth getAnother(long aNthObject) {
     if (Math.abs(aNthObject) > Integer.MAX_VALUE-2) throw new IllegalArgumentException("Počtu měsíců o něž posouvat je " + aNthObject + " a to je na mě v absolutní hodnotě moc!");
