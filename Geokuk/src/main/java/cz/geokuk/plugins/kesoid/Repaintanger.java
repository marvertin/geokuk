...
public class Repaintanger {
  ...
  public void include(Insets in) {
    insets = new Insets(
        Math.max(insets.top, in.top),
        Math.max(insets.left, in.left),
        Math.max(insets.bottom, in.bottom), 
        Math.max(insets.right, in.right));

  }

  public void include(Imagant imagant) {
    if (imagant == null) return;
    Insets insets = new Insets(-imagant.getYpos(), -imagant.getXpos(),
        imagant.getYpos() + imagant.getImage().getHeight(), 
        imagant.getXpos() + imagant.getImage().getWidth());
    include(insets);
  }
