 ...
 *
 * Statické metody obsahují, co lez dělat přímo.
 * Instanční, to co souvisí s navázaností na Coord.
 *
 * @author tatinek
 *
 */
public class Kaputer {
	...
	public Kaputer(Coord aCoord) {
		...
		for (x0 = xn / 2 - xd; x0 > 0; x0 -= KACHLE_PIXELS); // nastavit x0 i xx zleva před kreslenou plochu (- je zde, protože nalevo od středu)
		for (y0 = yn / 2 + yd; y0 > 0; y0 -= KACHLE_PIXELS); // nastavit x0 i yy shora před kreslenou plochu (+ je zde, protože dolů od středu, druhé plus, ptotož mouy jde sdola nahoru)
		assert x0 <=0 && y0 <= 0;
	...
	/**
	 * Bod, na který se má vykreslit levý horní roh kachle pro zadané indexy uvnitř výřezu.
	 * Kachle [0,0] vždy postihne levý horní roh výřezu.
	 * Kachle [pocetKachliX - 1, pocetKachliY -1 ] vždy postihne pravý dolní roh výřezu.
	 * @param xi index kachle ve vřezu 0 .. pocetKachliX-1
	 * @param yi index kachle ve vřezu 0 .. pocetKachliY-1
	 */
