...
class CacheNaKachleDisk {
    ...
    private static final int MAXIMALNI_VELIKOST_FRONTY_ZAPISUJICI_NA_DISK = 100000;
    ...
    public CacheNaKachleDisk(KachleModel kachleModel) {
        ...
        km = KachleManagerFactory.getInstance(kachleModel.getKachleCacheFolderHolder());
    }
    ...
    private class DiskovyZapisovac implements Runnable {
        ...
        public void run() {
            for (; ; ) {
                try {
                    List<DiskSaveRequest> requests = new ArrayList<>();
                    naZapsaniNaDisk.drainTo(requests, MAXIMALNI_VELIKOST_FRONTY_ZAPISUJICI_NA_DISK);
                    if (!requests.isEmpty()) {
                        if (kachleModel.isUkladatMapyNaDisk()) {
                            km.save(requests);
                        }
                        pocitZapsanoNaDisk.add(requests.size());
